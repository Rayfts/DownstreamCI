...
export const harnesses: HarnessCapability[] = [
  ...
  },
  {
    id: "pi",
    displayName: "Pi",
    upstream: "mitsuhiko/pi-mono",
    automated: true,
    structuredOutput: true,
    verifiedAt,
    evidence: ["packages/coding-agent/docs/usage.md", "packages/coding-agent/docs/rpc.md"],
    notes: "Uses print mode plus --mode json, which the current upstream usage docs define as JSON-lines event output.",
  ...
  },
  {
    ...
    structuredOutput: true,
    verifiedAt,
    evidence: ["documentation/docs/guides/running-tasks.md"],
    notes: "Uses documented non-interactive stream-json output and --no-session so CI analysis does not persist a Goose session.",
    buildInvocation: (prompt) => invocation("goose", ["run", "--output-format", "stream-json", "--no-session", "--text", prompt], "jsonl"),
  },
  {
    id: "cline",
    displayName: "Cline",
    ...
    structuredOutput: true,
    verifiedAt,
    evidence: ["apps/cli/README.md", "apps/cli/src/main.ts"],
