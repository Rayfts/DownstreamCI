...
Treat Docker daemon access on coordinator/worker hosts as privileged infrastructure. Do not expose the daemon socket to downstream containers. Restrict `/internal/*` to trusted workers/operators and rotate `DOWNSTREAMCI_INTERNAL_TOKEN` if it is exposed.
