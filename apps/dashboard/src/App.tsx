      ...
      {run ? (
        <>
          ...
          <section className="metrics">
            <article>
              <strong>{run.comparisons.length}</strong>
              <span>downstreams</span>
            </article>
            <article>
              <strong>{counts.unaffected ?? 0}</strong>
              <span>unaffected</span>
            </article>
            <article>
              <strong>{counts["baseline-failing"] ?? 0}</strong>
              <span>baseline-failing</span>
            </article>
            <article>
              <strong>{counts["newly-broken"] ?? 0}</strong>
              <span>newly-broken</span>
            </article>
          </section>
          <nav className="tabs">
            {(["matrix", "failures", "logs", "analysis", "runtime", "history"] as View[]).map((item) => (
              <button
                type="button"
  ...
  return (
    <section className="panel">
      <div className="panelTitle">
        <h2>Baseline / candidate matrix</h2>
        <span>Deterministic CI fact</span>
      </div>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          {comparisons.map((item) => (
            <tr key={comparisonKey(item)}>
              <td>
                {item.downstream?.repository ?? "downstream"}
                <small>{item.downstream?.ref}</small>
              </td>
      ...
      {failures.length ? (
        failures.map((item) => (
          <article className="panel" key={comparisonKey(item)}>
            <div className="panelTitle">
              <h2>{item.downstream?.repository ?? "downstream"}</h2>
              <code>{item.classification}</code>
            </div>
            <p>{item.reason}</p>
            {item.candidateSignature ? (
              <p>
                Signature <code>{item.candidateSignature}</code>
              </p>
            ) : null}
          </article>
        ))
        ...
        <article className="panel">
          <h2>No failures</h2>
          <p>Every approved downstream passed baseline and candidate.</p>
        </article>
      )}
    </section>
  );
      ...
      {comparisons.map((item) => (
        <article className="panel" key={comparisonKey(item)}>
          ...
          <div className="loggrid">
            <Log title="Baseline" result={item.baseline.test} />
            <Log title="Candidate" result={item.candidate.test} />
          </div>
        </article>
      ))}
    </section>
  ...
  return (
    <div>
      <div className="panelTitle">
        <strong>{title}</strong>
        <span>
          {result.durationMs} ms · exit {result.exitCode ?? "null"}
        </span>
      </div>
      <pre>{text.slice(-12000)}</pre>
    </div>
  );
}
  ...
  return (
    <section className="stack">
      {items.length ? (
        items.map((item) => (
          <article className="panel" key={comparisonKey(item)}>
            <p className="analysis">ANALYSIS · {item.analysis?.harness}</p>
            <h2>{item.downstream?.repository ?? "downstream"}</h2>
            <pre>{item.analysis?.raw}</pre>
          </article>
        ))
      ) : (
        <article className="panel">
          <h2>No agent analysis</h2>
          <p>Agent output only appears for candidate-only failures when a harness is explicitly selected.</p>
        </article>
      )}
    </section>
  );
  ...
  return (
    <section className="stack">
      {comparisons.map((item) => (
        <article className="panel" key={comparisonKey(item)}>
          <h2>{item.downstream?.repository ?? "downstream"}</h2>
          <pre>{JSON.stringify(item.candidate.environment, null, 2)}</pre>
        </article>
      ))}
    </section>
  );
        ...
        <tbody>
          {runs.map((run) => (
            <tr key={run.id}>
              ...
              <td>{run.comparisons.length}</td>
              <td>{run.comparisons.filter((item) => item.classification === "newly-broken").length}</td>
            </tr>
          ))}
        </tbody>
      </table>
